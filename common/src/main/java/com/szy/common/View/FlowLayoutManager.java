...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    public FlowLayoutManager removeItemPerLineLimit() {
        ...
        if (layoutHelper != null) {
            cacheHelper = new CacheHelper(FlowLayoutOptions.ITEM_PER_LINE_NO_LIMIT,
    ...
    public void onAttachedToWindow(final RecyclerView view) {
        ...
        cacheHelper = new CacheHelper(flowLayoutOptions.itemsPerLine,
                layoutHelper.visibleAreaWidth());
    ...
    private void addNewLineAtBottom(RecyclerView.Recycler recycler) {
        int x = layoutStartPoint().x, y = getDecoratedBottom(
                getChildAt(getMaxHeightLayoutPositionInLine(getChildCount() - 1)));
    ...
    private void addNewLineAtTop(RecyclerView.Recycler recycler) {
        int x = layoutStartPoint().x, bottom = getDecoratedTop(
                getChildAt(getMaxHeightLayoutPositionInLine(0))), y;
        int height = 0;
    ...
    private boolean calcChildLayoutRect(View child, int x, int y, int lineHeight, Rect rect) {
        return calcChildLayoutRect(child, x, y, lineHeight,
                LayoutContext.fromLayoutOptions(flowLayoutOptions), rect);
    }
    ...
    private boolean calcChildLayoutRect(View child, int x, int y, int lineHeight, LayoutContext layoutContext, Rect rect) {
        ...
        switch (layoutContext.layoutOptions.alignment) {
            case RIGHT:
                if (LayoutHelper.shouldStartNewline(x, childWidth, leftVisibleEdge(),
                        rightVisibleEdge(), layoutContext)) {
                    newLine = true;
    ...
    private boolean childVisible(boolean preLayout, int left, int top, int right, int bottom) {
        ...
        return Rect.intersects(new Rect(leftVisibleEdge(), topVisibleEdge(), rightVisibleEdge(),
                bottomVisibleEdge()), new Rect(left, top, right, bottom));
    }
    ...
    private int getMaxHeightLayoutPositionInLine(int layoutPosition) {
        ...
        int maxIndexBefore = layoutPosition, maxIndexAfter = layoutPosition, maxHeightBefore = getDecoratedMeasuredHeight(
                child), maxHeightAfter = getDecoratedMeasuredHeight(child);
    ...
    private int getOffsetOfItemToFirstChild(int adapterPosition, RecyclerView.Recycler recycler) {
        ...
        if (adapterPosition > firstChildPosition) {
            ...
            if (lastChildAdapterPosition >= adapterPosition) {
                ...
                int y = getDecoratedBottom(getChildAt(
                        getMaxHeightLayoutPositionInLine(getChildCount() - 1))) - topVisibleEdge();
    ...
    private boolean isEndOfLine(int index, LayoutContext layoutContext) {
        if (LayoutHelper.hasItemsPerLineLimit(
    ...
    private boolean lineVisible(int index) {
        ...
        return Rect.intersects(new Rect(leftVisibleEdge(), topVisibleEdge(), rightVisibleEdge(),
                        bottomVisibleEdge()),
                new Rect(leftVisibleEdge(), getDecoratedTop(maxHeightItem), rightVisibleEdge(),
                        getDecoratedBottom(maxHeightItem)));
    }
    ...
    private void onPreLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        Point point = layoutHelper.layoutStartPoint(
        ...
        while (currentItemPosition < state.getItemCount()) {
            ...
            if (!childRemoved) {
                real_newline = calcChildLayoutRect(child, real_x, real_y, real_height, afterContext,
                        real_rect);
                if (real_newline) {
            ...
            if (!childVisible(true, real_x, real_y, real_x + rect.width(),
                    real_y + rect.height())) {
