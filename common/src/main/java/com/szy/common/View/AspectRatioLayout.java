...
public class AspectRatioLayout extends FrameLayout {
    ...
    public void setLayoutParams(android.view.ViewGroup.LayoutParams params) {
        if (mMeasuredMode == MODE_MEASURE_INIT) {
            ...
            } else if (params.height != LayoutParams.WRAP_CONTENT && params.width == LayoutParams.WRAP_CONTENT) {
    ...
    private void init(Context c, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = c.obtainStyledAttributes(attrs, R.styleable.AspectRatioLayout);
            mWidthHeightAspectRatio = a.getFloat(R.styleable.AspectRatioLayout_widthHeightRatio, 0);
            mMeasuredMode = a.getInt(R.styleable.AspectRatioLayout_measureFirst, MODE_MEASURE_INIT);
            a.recycle();
        }
    }
}